...
- `coridata-anon-{ipHash}-{ts}` — anonymous caller with IP-based fingerprint
- `coridata-tag-{callerId}-{ts}` — tagged caller with explicit identifier

The query unpacks these into two columns: `caller_type` for aggregation
(`anonymous`, `tagged`, or `local`) and `caller_id` for drill-down (the IP
hash or explicit tag).

`anonymous` deliberately combines requests with no credentials at all and
vended credentials without a `caller` tag. Both answer "who did this?" with
nothing — the distinction (had a credential vs. didn't) doesn't matter for
attribution. On the public buckets, `anonymous` is usually the bulk of the
traffic. `local` is a separate category: a known AWS identity that didn't
go through the vending endpoint.

```{r chart-by-caller-type}
activity |>
...
For a deeper look, drill down into individual caller IDs. Anonymous callers
are grouped by an 8-character IP hash; tagged callers show their explicit tag.

```{r chart-by-caller-id}
activity |>
...
grouping repeat callers by origin without requiring identification. Passing
a `caller` tag moves a request into the `tagged` category with an explicit
identifier:
